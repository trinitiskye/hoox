...
export default function AdminLoginPage({ onNavigate, onLogin }: AdminLoginPageProps) {
  ...
  const handleCreateDemo = async () => {
    ...
    setLoading(false);
    if (success) {
      setDemoMsg('✅ Demo admin account ready! Use the credentials above to sign in.');
      setPassword('DemoAdmin123!');
    } else {
      setDemoMsg('❌ Error: ' + error);
    }
  };
    ...
    <div className="min-h-screen bg-blue-50 flex items-center justify-center px-4 py-12">
      <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-sm">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6">
          <p className="text-blue-700 font-semibold text-sm flex items-center gap-1.5 mb-2">
            <Key className="w-4 h-4" /> Demo Admin Credentials
          </p>
        ...
        <div className="text-center mb-5">
          <button onClick={() => onNavigate('login')} className="text-blue-600 text-sm hover:underline">
            Regular user login
          </button>
        </div>
